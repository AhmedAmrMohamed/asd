...
class cfrate:
    def __init__(self):
        ...
        self.__url    = 'http://codeforces.com/api/contest.ratingChanges?contestId={}'.format(self.id)
        self.handle = None
                ...
                if self.handle:
                    for result in res['result']:
                        if result['handle'].lower()==self.handle:
                            ...
                            print('gain : {}\n{} -> {}'.format(new-old,old,new))
                            break
                    else:
                        print('couldn\'t find this handle ->{}'.format(self.handle))
                quit()
